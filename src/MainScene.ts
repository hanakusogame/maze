...
export class MainScene extends g.Scene {
	...
	constructor(param: g.SceneParameterObject) {
		...
		this.loaded.add(() => {
			...
			//操作方法選択
			const sprSelect = new g.Sprite({ scene: this, src: this.assets["select"], x: 0, y: 210 });
			sprTitle.append(sprSelect);

			const sprCursors: g.Sprite[] = [];
			for (let i = 0; i < 3; i++) {
				const sprCursor = new g.Sprite({ scene: this, src: this.assets["cursor"], x: 98 + (117 * i), y: 0, touchable:true });
				sprSelect.append(sprCursor);
				sprCursor.pointDown.add(() => {
					sprCursors.forEach(e => {
						e.opacity = 0;
						e.modified();
					});
					sprCursor.opacity = 1;
